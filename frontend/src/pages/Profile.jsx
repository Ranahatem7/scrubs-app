...
export default function Profile() {
  ...
  const s = {
    ...
    layout: {
      ...
      padding: `40px ${theme.pad}px 80px`, alignItems: "start",
    },
    account: {
      ...
      padding: 24, position: isDesktop ? "sticky" : "static", top: theme.barH + 20,
      order: isDesktop ? 2 : -1,
    },
    avatarRow: { display: "flex", alignItems: "center", gap: 14 },
    avatar: {
      width: 52, height: 52, borderRadius: "50%", background: theme.accent,
      display: "grid", placeItems: "center", fontFamily: theme.fontDisplay,
      fontSize: 22, fontWeight: 700, color: theme.textOnDark, flexShrink: 0,
    },
    accountName: { ...strongText("light"), fontFamily: theme.fontDisplay, fontSize: 19, margin: 0, lineHeight: 1.25 },
    accountEmail: { fontSize: 12, color: theme.textOnLightMuted, margin: "3px 0 0", wordBreak: "break-word" },
    memberSince: { fontSize: 11, color: theme.textOnLightMuted, letterSpacing: "0.06em", paddingTop: 16, marginTop: 16, borderTop: `1px solid ${theme.hairlineOnLight}` },
    signOutBtn: { ...btnGhost("light"), width: "100%", justifyContent: "center", marginTop: 18 },
    ordersCol: { order: isDesktop ? 1 : 0 },
    sectionHead: { display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 16 },
    sectionTitle: { fontSize: 11, fontWeight: 500, letterSpacing: "0.26em", textTransform: "uppercase", color: theme.accent, margin: 0 },
    orderCountNote: { fontSize: 12, color: theme.textOnLightMuted },
    stateBlock: {
      padding: "48px 24px", textAlign: "center", background: theme.surfaceLight,
    ...
    orderCard: {
      display: "flex", gap: 14, padding: 16, background: theme.surfaceLight,
      border: `1px solid ${theme.hairlineOnLight}`, borderRadius: theme.radius,
      boxShadow: "0 1px 3px rgba(34,37,42,0.06)",
    },
    orderThumb: { width: 56, height: 68, objectFit: "cover", borderRadius: 6, flexShrink: 0, background: theme.surfaceMuted },
    ...
    statusPill: (status) => ({
      flexShrink: 0, fontSize: 9, fontWeight: 500, letterSpacing: "0.12em",
    ...
    orderItemsText: { fontSize: 13, color: theme.textOnLightMuted, margin: "8px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
    orderFoot: { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, paddingTop: 10, borderTop: `1px solid ${theme.hairlineOnLight}` },
        ...
        <div style={s.ordersCol}>
          <div style={s.sectionHead}>
            ...
            {!ordersError && orders.length > 0 && (
              <span style={s.orderCountNote}>{orders.length} {orders.length === 1 ? "order" : "orders"}</span>
            )}
          </div>
          ...
          {ordersError && (
            <div style={s.stateBlock}>
              ...
              <button style={s.retryBtn} onClick={() => setReloadKey((k) => k + 1)}>Try again</button>
            </div>
          )}
              ...
              {orders.map((order) => {
                ...
                return (
                  <div key={order._id} style={s.orderCard}>
                    {order.items[0]?.image && <img src={order.items[0].image} alt="" style={s.orderThumb} />}
                    <div style={s.orderBody}>
                      <div style={s.orderHead}>
                        <div style={s.orderRefDate}>
                          ...
                          <p style={s.orderDate}>{new Date(order.createdAt).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}</p>
                        </div>
                        <span style={s.statusPill(order.status)}>{order.status}</span>
                      </div>
                      <p style={s.orderItemsText}>{order.items.map((i) => i.name).join(", ")}</p>
